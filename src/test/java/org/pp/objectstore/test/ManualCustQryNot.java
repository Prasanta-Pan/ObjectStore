...
public class ManualCustQryNot extends AbstractManualQuery<CustomerOrder> {
	/**
	 * 
	 * @param numOfParams
	 * @param set
	 */
	public ManualCustQryNot(int numOfParams, NavigableSet<CustomerOrder> set) {
		super(numOfParams, set);
		
	}
	// "customerEmail = ? && orderDate between ? && (totalOrder > ? || numOfUnits > ?)"
	...
	public List<CustomerOrder> list() {
		...
		for (Iterator<CustomerOrder> itr = lSet.iterator(); itr.hasNext();) {
			...
			if (co.getTotalOrder() > (double) params[2] || co.getNumOfUnits() > (int) params[3])
				continue;
			// add to list
			fl.add(co);
		}
