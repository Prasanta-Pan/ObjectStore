...
import static org.pp.objectstore.test.Util.ages;
import static org.pp.objectstore.test.Util.booleans;
import static org.pp.objectstore.test.Util.emails;
import static org.pp.objectstore.test.Util.genRandChar;
import static org.pp.objectstore.test.Util.genRandDouble;
import static org.pp.objectstore.test.Util.genRandFloat;
import static org.pp.objectstore.test.Util.genRandShort;
import static org.pp.objectstore.test.Util.millisToDateString;
import static org.pp.objectstore.test.Util.names;
import static org.pp.objectstore.test.Util.randomDateToMilis;
...
public class CustomerOrder implements Comparable<CustomerOrder> {
	...
	public static final CustomerOrder generate() throws Exception {
		...
		String custName = names[num];
		// random customer email
		String custEmail = emails[num];
		// random age
		byte age = ages[num];
		// random sex
		boolean sex = booleans[num];
		// random unit
		short unit = genRandShort();
		// random total
		double total = genRandDouble();
		// random price
		float price = genRandFloat();
		// random record status
		char status = genRandChar();
		// random date
		long date = randomDateToMilis();
	...
	public void update() {
		// random unit
		numOfUnits = genRandShort();
		// random total
		totalOrder = genRandDouble();
		// random price
		unitPrice = genRandFloat();
		// random record status
		recordStatus = genRandChar();
	}
