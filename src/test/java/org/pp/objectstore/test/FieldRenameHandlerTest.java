...
public class FieldRenameHandlerTest extends AbstractTest {
	/**
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private void renameField(String className) throws Exception {
		// standard error message
		String msg = "Wealth field value supposed not to be zero";
		openDatabase();
		try {
			// load class
			Class custClass = new CustomerClassLoader(AddSortKeyTest.class.getClassLoader()).loadClass(PKG + className);
			// rename field
			factory.renameStoreFields(custClass, new FieldRenameHandler() {
				
				@Override
				public String newFieldName(String existingFieldName) {
					// only return new name if specified name has been changed
					if ("wealth".equals(existingFieldName))
						return "noWealth";
					// no changes for the rest of field
					return null;
				}
			});
			// open store
