...
public class AddNewFieldTest extends AbstractTest {
	/**
	...
	/**
	 * According to design Modification Date field values must be zero
	 * @param className
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private void verifyFieldValues(String className) throws Exception {
		// standard error message
		String msg = "Modification Date field value supposed to be zero";
		openDatabase();
		try {
    		// load class
			Class custClass = new CustomerClassLoader(AddSortKeyTest.class.getClassLoader()).loadClass(PKG + className);
